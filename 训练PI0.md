以命令 `pi05_libero --num-train-steps=10` 为准，梳理“CUDA_VISIBLE_DEVICES=0 WANDB_MODE=offline uv run scripts/train.py pi05_libero --exp-name=test_openpi --num-train-steps=10 --overwrite”的训练整体逻辑。其实际走的是 **LeRobot / Torch data loader 分支**，不是 RLDS 分支。

**1. 初始化 Config**

1.1. Shell 环境生效  
代码：[train.py 入口](scripts/train.py)（第 313 行）  
...
代码：[config.py cli](src/openpi/training/config.py)（第 1218 行）  
...
1.4. 命令行参数覆盖默认配置  
代码：[TrainConfig](src/openpi/training/config.py)（第 610 行）  
...
代码：[pi05_libero 配置](src/openpi/training/config.py)（第 868 行）  
...
代码：[main JAX cache](scripts/train.py)（第 260 行）  
...
2.4. 创建随机数 key  
代码：[main rng](scripts/train.py)（第 265 行）  
...
代码：[main mesh/sharding](scripts/train.py)（第 271 行）  
`mesh` 描述当前有哪些设备参与训练。`sharding` 描述数组如何分布到设备上。单卡时基本可以理解为“都放到这一张 GPU 上”；多卡时才涉及数据并行或参数分片。

2.6. 初始化 checkpoint 目录  
代码：[initialize_checkpoint_dir](src/openpi/training/checkpoints.py)（第 20 行）  
路径是：
...
2.7. 初始化 wandb  
代码：[init_wandb](scripts/train.py)（第 43 行）  
wandb 负责记录训练指标。由于 `WANDB_MODE=offline`，日志会本地保存，不上传。

**3. 创建 Data Loader**

3.1. 调用 `create_data_loader(config, ...)`  
代码：[create_data_loader](src/openpi/training/data_loader.py)（第 223 行）  
Data Loader 是训练数据加载器，负责读取样本、做 batch、转换格式。先后分别是repack、LiberoInputs、Normalize、ModelTransformFactory
它先执行：
...
代码：[LeRobotLiberoDataConfig.create](src/openpi/training/config.py)（第 334 行）  
...
代码：[RepackTransform](src/openpi/transforms.py)（第 80 行）  
...
代码：[LiberoInputs](src/openpi/policies/libero_policy.py)（第 30 行）  
...
同时配置 [LiberoOutputs](src/openpi/policies/libero_policy.py)（第 78 行），但训练时主要用 inputs，outputs 更偏推理阶段。

3.5. 配置 `model_transforms`  
代码：[ModelTransformFactory](src/openpi/training/config.py)（第 104 行）  
调用：
...
对应代码：[InjectDefaultPrompt](src/openpi/transforms.py)（第 105 行）、[ResizeImages](src/openpi/transforms.py)（第 195 行）、[TokenizePrompt](src/openpi/transforms.py)（第 266 行）、[PadStatesAndActions](src/openpi/transforms.py)（第 347 行）。

3.6. `create_data_loader` 判断走哪个 data loader 分支  
代码：[create_data_loader 分支](src/openpi/training/data_loader.py)（第 245 行）  
判断条件是：
...
代码：[create_torch_data_loader](src/openpi/training/data_loader.py)（第 303 行）  
...
代码：[create_torch_dataset](src/openpi/training/data_loader.py)（第 130 行）  
...
代码：[transform_dataset](src/openpi/training/data_loader.py)（第 172 行）  
每个样本会按顺序走：
...
对应代码：[PromptFromLeRobotTask](src/openpi/transforms.py)（第 329 行）、[Normalize](src/openpi/transforms.py)（第 115 行）。

3.10. `TorchDataLoader` 负责 batch 和 shuffle  
代码：[TorchDataLoader](src/openpi/training/data_loader.py)（第 413 行）  
底层用 `torch.utils.data.DataLoader`。虽然名字里有 Torch，但当前 `framework="jax"`，最后 batch 会转成 JAX array。

3.11. `DataLoaderImpl` 输出训练需要的 batch  
代码：[DataLoaderImpl](src/openpi/training/data_loader.py)（第 562 行）  
最终 yield：
...
`Observation` 是模型输入的数据结构。代码：[Observation.from_dict](src/openpi/models/model.py)（第 110 行）。训练 step 收到：
...
代码：[create_rlds_data_loader](src/openpi/training/data_loader.py)（第 372 行）  
...
RLDSDataLoader 主要用于已经按 batch 组织好的 RLDS / DROID 数据。代码：[RLDSDataLoader](src/openpi/training/data_loader.py)（第 518 行）。

**4. 初始化模型和 TrainState**

4.1. 创建 optimizer  
代码：[init_train_state](scripts/train.py)（第 86 行）、[create_optimizer](src/openpi/training/optimizer.py)（第 105 行）  
...
4.2. 创建模型  
代码：[Pi0Config.create](src/openpi/models/pi0_config.py)（第 49 行）  
调用：
...
对 `pi05_libero` 来说，`config.model = Pi0Config(pi05=True, ...)`，所以创建的是 [Pi0](src/openpi/models/pi0.py)（第 66 行），不是 `Pi0Faster`。

4.3. `eval_shape` 得到 `train_state_shape`  
代码：[init_train_state eval_shape](scripts/train.py)（第 125 行）  
`eval_shape` 只推导模型参数和训练状态的形状，不真正做完整计算。它用来知道参数树长什么样，方便后续加载 checkpoint 和设置 sharding。

4.4. 根据 shape 创建 sharding  
代码：[init_train_state sharding](scripts/train.py)（第 128 行）  
...
代码：[_load_weights_and_validate](scripts/train.py)（第 74 行）  
`pi05_libero` 使用：
...
4.7. 把预训练权重合进模型  
代码：[init_train_state 合并权重](scripts/train.py)（第 99 行）  
已有 checkpoint 参数会覆盖初始化参数。没有被 checkpoint 覆盖的参数保留初始化值。

4.8. 创建 `TrainState`  
代码：[TrainState 创建](scripts/train.py)（第 108 行）  
...
代码：[restore_state](src/openpi/training/checkpoints.py)（第 87 行）  
resume 是继续训练。你的命令用了 `--overwrite`，所以不是 resume。

**5. 编译单步训练函数**

5.1. 定义原始 `train_step`  
代码：[train_step](scripts/train.py)（第 146 行）  
`train_step` 是一次参数更新的 Python 逻辑。

5.2. 用 `functools.partial(train_step, config)` 固定 config  
代码：[ptrain_step 创建](scripts/train.py)（第 284 行）  
...
5.4. 设置输入输出 sharding  
代码：[ptrain_step sharding](scripts/train.py)（第 286 行）  
告诉 JAX：
...
6.1. 设置起始 step  
代码：[start_step](scripts/train.py)（第 293 行）  
非 resume 时：
...
6.2. 创建进度条 `pbar`  
代码：[pbar](scripts/train.py)（第 294 行）  
...
代码：[训练循环调用 ptrain_step](scripts/train.py)（第 304 行）  
核心语句：
...
6.4. 收集训练指标  
代码：[infos 收集](scripts/train.py)（第 305 行）  
`info` 包含：
...
代码：[训练日志](scripts/train.py)（第 306 行）  
默认 `log_interval=100`。因为你只跑 10 step，所以通常只在 step 0 打一次日志。

6.6. 取下一个 batch  
代码：[next batch](scripts/train.py)（第 313 行）  
每轮训练结束后：
...
6.7. 保存 checkpoint  
代码：[save_state](src/openpi/training/checkpoints.py)（第 65 行）  
...
7.2. 设置训练模式  
代码：[model.train](scripts/train.py)（第 153 行）  
调用：
...
7.3. 定义 `loss_fn`  
代码：[loss_fn](scripts/train.py)（第 156 行）  
`loss_fn` 内部调用：
...
代码：[fold_in](scripts/train.py)（第 169 行）  
用：
...
7.5. 拆出 batch  
代码：[batch unpack](scripts/train.py)（第 174 行）  
得到：
...
7.6. 指定可训练参数  
代码：[DiffState](scripts/train.py)（第 179 行）  
通过 `config.trainable_filter` 排除 frozen 参数。frozen 是冻结参数，即不更新。

7.7. 前向计算 loss 并求梯度  
代码：[value_and_grad](scripts/train.py)（第 184 行）  
调用：
...
代码：[optimizer update](scripts/train.py)（第 187 行）  
根据梯度和 optimizer state 得到参数更新量。

7.9. 应用 updates  
代码：[apply_updates](scripts/train.py)（第 191 行）  
更新当前可训练参数。

7.10. 写回模型并导出完整参数树  
代码：[nnx.update](scripts/train.py)（第 196 行）  
包含训练参数和 frozen 参数。

7.11. 更新 `TrainState`  
代码：[new_state](scripts/train.py)（第 201 行）  
step 加 1，并保存新的 params / optimizer state。

7.12. 更新 EMA 参数  
代码：[EMA 更新](scripts/train.py)（第 203 行）  
`ema_decay=0.999`，所以维护一份平滑后的参数。

7.13. 统计日志指标  
代码：[info](scripts/train.py)（第 224 行）  
返回：
...
8.1. 输入真实 actions  
代码：[Pi0.compute_loss](src/openpi/models/pi0.py)（第 189 行）  
形状大致是：
...
8.2. 随机采样 noise  
代码：[noise](src/openpi/models/pi0.py)（第 196 行）  
noise 是噪声。这里生成和 actions 一样形状的高斯噪声。

8.3. 随机采样 time  
代码：[time](src/openpi/models/pi0.py)（第 197 行）  
每个样本采一个 flow matching 时间点。flow matching 是让模型学习从噪声流向真实数据的速度场。

8.4. 构造 noisy action  
代码：[x_t](src/openpi/models/pi0.py)（第 199 行）  
公式：
...
8.5. 构造监督目标速度  
代码：[u_t](src/openpi/models/pi0.py)（第 200 行）  
公式：
...
代码：[embed_prefix](src/openpi/models/pi0.py)（第 106 行）  
图像、文本、state 进入 `embed_prefix`。prefix 是条件信息，即“当前看到了什么、任务是什么”。

8.7. 编码 action suffix  
代码：[embed_suffix](src/openpi/models/pi0.py)（第 140 行）  
`x_t` 和 `time` 进入 `embed_suffix`。suffix 是动作生成相关的 token。

8.8. 模型 forward  
代码：[PaliGemma forward](src/openpi/models/pi0.py)（第 207 行）  
forward 是前向计算，即模型根据输入算输出。PaliGemma / action expert 输出 action token 表征。

8.9. 投影得到预测速度  
代码：[action_out_proj](src/openpi/models/pi0.py)（第 210 行）  
通过：
...
8.10. 计算 MSE loss  
代码：[loss return](src/openpi/models/pi0.py)（第 212 行）  
