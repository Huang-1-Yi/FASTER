...
```text
主线 A：普通完整 chunk 推理
PI0 / PI0.5 / Pi0Faster 都可以走这条路。

主线 B：Pi0Faster streaming 推理
只有实现 sample_actions_streaming_init/step 的 Pi0Faster 才能走这条路。
```

### 2.1 普通完整 chunk 推理

普通 PI0 / Pi0Faster 的“非 streaming 推理”共享同一个外壳。这里的“非 streaming”特指调用 [Policy.infer](C:/QClaw/FASTER_hy/src/openpi/policies/policy.py:77) 和模型的 `sample_actions()`，等待完整 action chunk 生成后一次性返回；`Pi0Faster` 额外支持 streaming，但它不是只能 streaming。
...
### 2.2 Pi0Faster streaming 推理

Pi0Faster 的 streaming 推理不是走 `sample_actions()`，而是把一次完整 denoising 拆成 `init + 多次 step`。这条链路的目标不是改变最终 action 表示，而是在 HAS 过程中一旦某些近端 action ready，就先经过输出 transform 并发给客户端。
...
-> Pi0Faster.sample_actions_streaming_init
   输出：noise, already_output, t_starts, dt_schedule, is_ready_after_step,
        kv_cache, prefix_mask, prefix_action_mask, action_prefix, observation
-> Python host loop for each denoising step:
   Pi0Faster.sample_actions_streaming_step
   输出：x_next, already_output_next, newly_ready
   if newly_ready:
      select ready actions from x_next
      -> output transforms
      -> on_actions_ready callback
      -> websocket partial message
-> final x_t
-> output transforms
...
| 环节 | 函数 | 输入 | 输出 |
|---|---|---|---|
| streaming server 接收 obs | [StreamingWebsocketPolicyServer._handler](C:/QClaw/FASTER_hy/src/openpi/serving/websocket_policy_server.py:134) | WebSocket msgpack observation | 后台线程中的 `policy.infer_streaming(obs)` |
| streaming policy 入口 | [Policy.infer_streaming](C:/QClaw/FASTER_hy/src/openpi/policies/policy.py:127) | 原始 obs dict, optional callback | final action dict，并通过 callback 发 partial actions |
| streaming 初始化 | [sample_actions_streaming_init](C:/QClaw/FASTER_hy/src/openpi/models/pi0_faster.py:365) | rng, `Observation`, `num_steps`, optional `delay/action_prefix` | HAS 时间表、KV cache、ready mask、初始 noise |
| 单步 denoising | [sample_actions_streaming_step](C:/QClaw/FASTER_hy/src/openpi/models/pi0_faster.py:419) | `x_t`, 当前 `t_curr/dt_curr`, cache, masks | `x_next`, `already_output_next`, `newly_ready` |
| partial 输出 transform | [Policy.infer_streaming callback 处理](C:/QClaw/FASTER_hy/src/openpi/policies/policy.py:218) | newly ready 的 normalized actions | 环境尺度/维度的 partial actions |
| websocket partial 发送 | [on_actions_ready](C:/QClaw/FASTER_hy/src/openpi/serving/websocket_policy_server.py:149) | transformed ready actions | `{"type": "partial", "actions": ready_actions}` |
| websocket final 发送 | [final_result](C:/QClaw/FASTER_hy/src/openpi/serving/websocket_policy_server.py:173) | final transformed output dict | `{"type": "final", ...}` |

所以总调用流应理解为：`Pi0Faster` 既能作为普通连续 action 模型被 `Policy.infer()` 调用，也能作为实时流式模型被 `Policy.infer_streaming()` 调用。前者服务于完整 chunk 评估和兼容普通 policy 接口；后者才体现 FASTER 论文的实时反应优势。
